...
class DatasetResult:
    ...
    shuffle_perm: Optional[torch.LongTensor] = None

    def __iter__(self):
        return iter((self.train_loader, self.val_loader, self.test_loader, self.input_dim))
...
def generate_feature_shuffle_permutation(n_features, shuffle_pct, seed=None):
    """Generate a permutation that shuffles shuffle_pct% of feature indices.

    Uses an isolated torch.Generator so it does not affect the global RNG.
    Returns torch.arange(n_features) (identity) when shuffle_pct <= 0.
    Returns a full random permutation when shuffle_pct >= 100.
    """
    ...
    if shuffle_pct >= 100:
        ...
        return torch.randperm(n_features, generator=gen)

    k = round(n_features * shuffle_pct / 100)
    if k < 2:
        return perm
    ...
    # Select k indices to shuffle
    chosen = torch.randperm(n_features, generator=gen)[:k]
    # Permute those indices among themselves
    shuffled = chosen[torch.randperm(k, generator=gen)]
    perm[chosen] = shuffled
    return perm


def apply_feature_shuffle(X, perm):
    """Apply a column permutation to all rows of X."""
    return X[:, perm]


def load_mnist(seed, n_train, n_valtest, device, batch_size, shuffle_pct=0, shuffle_seed=None):
    ...
    # Feature shuffling
    perm = generate_feature_shuffle_permutation(input_dim, shuffle_pct, seed=shuffle_seed)
    X_train = apply_feature_shuffle(X_train, perm)
    X_test = apply_feature_shuffle(X_test, perm)

    # Save raw CPU tensors for QE computation (before device transfer)
    ...
    # Extract val/test raw tensors using split indices
    ...
    return DatasetResult(
        ...
        X_train=X_train_raw,
        y_train=y_train_raw,
        X_val=X_test_raw[val_indices],
        y_val=y_test_raw[val_indices],
...
def load_cifar(seed, n_train, n_valtest, device, batch_size, shuffle_pct=0, shuffle_seed=None):
    # Set random seed for reproducibility
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(seed)
...
def load_coco(seed, n_train, n_valtest, device, batch_size, num_classes=80, img_size=224, shuffle_pct=0, shuffle_seed=None):
    """
    ...
    # Feature shuffling
    perm = generate_feature_shuffle_permutation(input_dim, shuffle_pct, seed=shuffle_seed)
    X_flat = apply_feature_shuffle(X_flat, perm)
    ...
    # Split into train and val/test (save raw CPU tensors before device transfer)
    X_train_raw = X_flat[:n_train].clone()
    y_train_raw = y[:n_train].clone()
    X_valtest_raw = X_flat[n_train:n_train + n_valtest].clone()
    y_valtest_raw = y[n_train:n_train + n_valtest].clone()
    ...
    return DatasetResult(
        ...
        X_train=X_train_raw,
        y_train=y_train_raw,
        X_val=X_valtest_raw[val_indices],
        y_val=y_valtest_raw[val_indices],
        X_test=X_valtest_raw[test_indices],
        y_test=y_valtest_raw[test_indices],
        shuffle_perm=perm,
    )
...
def load_data(dataset_name, batch_size, num_workers=0, n_train=50000, n_valtest=10000,
    ...
    Returns a DatasetResult for datasets that support feature shuffling
    (mnist, cifar10, coco), or a plain 3-tuple of loaders for others.
    """
